...
ALTER TABLE `httc`.`requests` MODIFY COLUMN status ENUM('PENDING', 'ACCEPTED', 'REJECTED') NOT NULL DEFAULT 'PENDING' COMMENT 'Current status of the request';
...
CREATE INDEX idx_requests_status ON `httc`.`requests`(status);
