"use client";

import React, { useEffect, useState } from "react";
import { useUser } from "@/context/UserContext";
import { useServicesRequests } from "@/hooks/use-services-requests";
import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { IoArrowBack, IoFilter } from "react-icons/io5";
import { useRouter, useSearchParams } from "next/navigation";
import { cn, formatDateTime } from "@/lib/utils";
import { GroomingRequest } from "@/lib/api/services-requests";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { FaCheck, FaTimes } from "react-icons/fa";
import { toast } from "sonner";
import ServiceRequestActionModal from "@/components/services/ServiceRequestActionModal";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
  AlertDialogDescription,
  AlertDialogFooter,
  AlertDialogHeader,
  AlertDialogTitle,
} from "@/components/ui/alert-dialog";

const ServiceRequestsPage = () => {
  const { user, isLoading } = useUser();
  const {
    requests,
    loading,
    error,
    fetchMyRequests,
    searchRequestsList,
    updateRequestById,
  } = useServicesRequests();
  const router = useRouter();
  const searchParams = useSearchParams();
  const status = searchParams.get("status") || "ALL";
  const [activeModal, setActiveModal] = useState<{
    isOpen: boolean;
    type: "accept" | "schedule" | "start" | "complete" | "reject";
    requestId: number;
    defaultDateTime?: string;
  } | null>(null);
  const [cancelDialog, setCancelDialog] = useState<{
    isOpen: boolean;
    requestId: number;
  } | null>(null);

  useEffect(() => {
    if (!isLoading && !user) {
      router.push("/sign-in");
    }
  }, [user, isLoading, router]);

  useEffect(() => {
    if (user) {
      if (user.role === "ROLE_PET_OWNER") {
        fetchMyRequests();
      } else if (user.role === "ROLE_STAFF" || user.role === "ROLE_ADMIN") {
        searchRequestsList({
          status: status === "ALL" ? undefined : (status as any),
        });
      }
    }
  }, [user, fetchMyRequests, searchRequestsList, status]);

  if (isLoading) {
    return <div>Loading user information...</div>;
  }

  if (!user) {
    return null;
  }

  const getStatusColor = (status: string) => {
    switch (status) {
      case "PENDING":
        return "bg-amber-400";
      case "APPROVED":
        return "bg-blue-400";
      case "REJECTED":
        return "bg-red-400";
      case "SCHEDULED":
        return "bg-indigo-400";
      case "IN_PROGRESS":
        return "bg-purple-400";
      case "COMPLETED":
        return "bg-lime-500";
      case "CANCELLED":
        return "bg-red-400";
      default:
        return "bg-gray-500";
    }
  };

  const handleStatusChange = (value: string) => {
    const params = new URLSearchParams(searchParams.toString());
    if (value === "ALL") {
      params.delete("status");
    } else {
      params.set("status", value);
    }
    router.push(`/dashboard/services/requests?${params.toString()}`);
  };

  const handleModalSubmit = async (data: {
    scheduledDateTime?: string;
    completedDateTime?: string;
    staffNotes?: string;
  }) => {
    if (!activeModal) return;

    try {
      const updateData: any = { staffNotes: data.staffNotes };

      switch (activeModal.type) {
        case "accept":
          updateData.status = "APPROVED";
          break;
        case "schedule":
          updateData.status = "SCHEDULED";
          updateData.scheduledDateTime = data.scheduledDateTime;
          break;
        case "start":
          updateData.status = "IN_PROGRESS";
          break;
        case "complete":
          updateData.status = "COMPLETED";
          updateData.completedDateTime = data.completedDateTime;
          break;
        case "reject":
          updateData.status = "REJECTED";
          break;
      }

      await updateRequestById(activeModal.requestId, updateData);
      toast.success(
        activeModal.type === "accept"
          ? "Request approved successfully"
          : activeModal.type === "schedule"
          ? "Service scheduled successfully"
          : activeModal.type === "start"
          ? "Service started successfully"
          : activeModal.type === "complete"
          ? "Service completed successfully"
          : "Request rejected successfully"
      );
      setActiveModal(null);
    } catch (error) {
      console.error("Failed to update request:", error);
      toast.error("Failed to update request");
    }
  };

  const handleCancel = async () => {
    if (!cancelDialog) return;

    try {
      await updateRequestById(cancelDialog.requestId, {
        status: "CANCELLED",
        staffNotes: "Cancelled by pet owner",
      });
      toast.success("Request cancelled successfully");
      setCancelDialog(null);
    } catch (error) {
      console.error("Failed to cancel request:", error);
      toast.error("Failed to cancel request");
    }
  };

  const getActionButtons = (request: GroomingRequest) => {
    if (user?.role === "ROLE_PET_OWNER") {
      if (["PENDING", "APPROVED", "SCHEDULED"].includes(request.status)) {
        return (
          <div className="mt-4">
            <Button
              onClick={() =>
                setCancelDialog({
                  isOpen: true,
                  requestId: request.id,
                })
              }
              className="border border-red-400 hover:bg-red-400 text-red-400 hover:text-white"
            >
              Cancel Request
            </Button>
          </div>
        );
      }
      return null;
    }

    if (user?.role !== "ROLE_STAFF" && user?.role !== "ROLE_ADMIN") {
      return null;
    }

    switch (request.status) {
      case "PENDING":
        return (
          <div className="mt-4 flex gap-2">
            <Button
              onClick={() =>
                setActiveModal({
                  isOpen: true,
                  type: "accept",
                  requestId: request.id,
                })
              }
              className="border border-lime-500 hover:bg-lime-500 hover:text-white text-lime-500"
            >
              <FaCheck className="" />
            </Button>
            <Button
              onClick={() =>
                setActiveModal({
                  isOpen: true,
                  type: "reject",
                  requestId: request.id,
                })
              }
              className="border border-red-400 hover:bg-red-400 text-red-400 hover:text-white"
            >
              <FaTimes className="" />
            </Button>
          </div>
        );
      case "APPROVED":
        return (
          <div className="mt-4 flex gap-2">
            <Button
              onClick={() =>
                setActiveModal({
                  isOpen: true,
                  type: "schedule",
                  requestId: request.id,
                  defaultDateTime: request.requestedDateTime,
                })
              }
              className="border border-indigo-500 hover:bg-indigo-500 hover:text-white text-indigo-500"
            >
              Schedule
            </Button>
          </div>
        );
      case "SCHEDULED":
        return (
          <div className="mt-4 flex gap-2">
            <Button
              onClick={() =>
                setActiveModal({
                  isOpen: true,
                  type: "start",
                  requestId: request.id,
                })
              }
              className="border border-purple-500 hover:bg-purple-500 hover:text-white text-purple-500"
            >
              Start Service
            </Button>
          </div>
        );
      case "IN_PROGRESS":
        return (
          <div className="mt-4 flex gap-2">
            <Button
              onClick={() =>
                setActiveModal({
                  isOpen: true,
                  type: "complete",
                  requestId: request.id,
                })
              }
              className="border border-lime-500 hover:bg-lime-500 hover:text-white text-lime-500"
            >
              Complete
            </Button>
          </div>
        );
      default:
        return null;
    }
  };

  return (
    <div className="container mx-auto py-6">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            className="hover:bg-brand-100 hover:text-white cursor-pointer rounded-full"
            onClick={() => router.push("/dashboard/services")}
          >
            <IoArrowBack className="w-6 h-6" />
          </Button>
          <h1 className="text-3xl font-bold">Service Requests</h1>
        </div>
        {(user.role === "ROLE_STAFF" || user.role === "ROLE_ADMIN") && (
          <div className="w-min px-2">
            <Select value={status} onValueChange={handleStatusChange}>
              <SelectTrigger className="w-full px-4 py-2 font-semibold border-none rounded-full shadow-sm">
                <IoFilter className="w-6 h-6 mr-2" />
                <SelectValue placeholder="Filter by status" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="ALL">All</SelectItem>
                <SelectItem value="PENDING">Pending</SelectItem>
                <SelectItem value="APPROVED">Approved</SelectItem>
                <SelectItem value="REJECTED">Rejected</SelectItem>
                <SelectItem value="SCHEDULED">Scheduled</SelectItem>
                <SelectItem value="IN_PROGRESS">In Progress</SelectItem>
                <SelectItem value="COMPLETED">Completed</SelectItem>
                <SelectItem value="CANCELLED">Cancelled</SelectItem>
              </SelectContent>
            </Select>
          </div>
        )}
      </div>

      {loading && <div>Loading...</div>}
      {error && <div className="text-red-500">{error}</div>}

      <div className="flex flex-col gap-8">
        {requests.map((request: GroomingRequest) => {
          const date = new Date(
            request.scheduledDateTime
              ? request.scheduledDateTime
              : request.requestedDateTime
          );
          const month = date.toLocaleString("default", { month: "long" });
          const year = date.getFullYear();
          const day = date.getDate();
          const time = date.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          });

          return (
            <Card
              key={request.id}
              className="flex flex-row p-0 bg-white rounded-2xl relative overflow-hidden shadow-lg border-none min-h-[200px]"
            >
              {/* Status badge at top right */}
              <div className="absolute right-8 top-5 text-slate-100 flex flex-col items-end">
                <Badge
                  className={cn(
                    getStatusColor(request.status),
                    "rounded-full px-3 py-1 font-bold tracking-wider shadow-sm"
                  )}
                >
                  {request.status}
                </Badge>
                {getActionButtons(request)}
              </div>

              {/* Left section: Date */}
              <div className="bg-slate-200 flex flex-col items-center justify-between py-6 px-6 w-36 min-w-[8rem] text-center">
                <div>
                  <div className="text-gray-800 text-lg font-mono flex flex-col items-center justify-between">
                    <span>{year}</span>
                    <span>{month}</span>
                  </div>
                  <div className="text-4xl font-bold text-gray-800 mt-1">
                    {day}
                  </div>
                </div>
                <div className="text-gray-700 text-sm font-mono mt-4">
                  {time}
                </div>
              </div>

              {/* Right section: Content */}
              <div className="flex-1 p-4 flex flex-col justify-between bg-white">
                <div className="text-xl font-mono mb-2 ml-0">
                  {request.service.name} for{" "}
                  <span className="text-2xl font-semibold">
                    {request.pet.name}
                  </span>
                </div>
                <div className="flex flex-col gap-2 text-gray-500 text-base mb-4 font-mono">
                  <div className="flex flex-row gap-2 items-center">
                    Requested at{" "}
                    <span className="font-semibold text-gray-700">
                      {formatDateTime(request.createdAt).dateTime}
                    </span>
                  </div>
                  {request.completedDateTime && (
                    <div className="flex flex-row gap-2 items-center">
                      Completed at{" "}
                      <span className="font-semibold text-gray-700">
                        {formatDateTime(request.completedDateTime).dateTime}
                      </span>
                    </div>
                  )}
                </div>
                {request.notes && (
                  <div className="flex flex-col gap-1">
                    <span className="font-semibold text-gray-500">Notes:</span>
                    <span className="text-gray-700 whitespace-pre-line">
                      {request.notes}
                    </span>
                  </div>
                )}
                {request.staffNotes && (
                  <div className="text-gray-600 mt-2">
                    <span className="font-semibold">Staff Notes:</span>{" "}
                    <span className="italic">{request.staffNotes}</span>
                  </div>
                )}
              </div>

              <div
                className={cn(
                  "min-h-full w-3 rounded-full absolute left-0",
                  getStatusColor(request.status)
                )}
              ></div>
            </Card>
          );
        })}
      </div>

      <ServiceRequestActionModal
        isOpen={activeModal?.isOpen || false}
        onClose={() => setActiveModal(null)}
        onSubmit={handleModalSubmit}
        type={activeModal?.type || "accept"}
        defaultDateTime={activeModal?.defaultDateTime}
      />

      <AlertDialog
        open={cancelDialog?.isOpen || false}
        onOpenChange={(open) => !open && setCancelDialog(null)}
      >
        <AlertDialogContent className="bg-white border-none rounded-2xl shadow-md">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-2xl font-bold">
              Cancel Request
            </AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to cancel this service request? This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="border-none">
              No, keep it
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={handleCancel}
              className="bg-brand hover:bg-brand-100 text-white"
            >
              Yes, cancel request
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
  );
};

export default ServiceRequestsPage;
